...
export default function EmailVerification() {
  ...
  const { user, setUser, emailToVerify, emailAvailable } = useAuth();
  const navigate = useNavigate();

  const { sendOtp, isSendingOtp, data } = useSendOtp();
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-blue-600 font-medium">{emailToVerify}</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border p-8">
          <div className="space-y-6">
            ...
            <button
              onClick={() => {
                (code.every((digit) => digit !== "") && code.join("").length === 6) && handleVerify(code.join(""))
              }}
